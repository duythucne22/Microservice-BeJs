...
exports.getZoneWithEventDetails = async (req, res) => {
  try {
    const zone = await EventZone.getZoneWithEventDetails(req.params.id);
    if (!zone) {
      return res.status(404).json({ success: false, message: 'Zone not found' });
    }
    res.status(200).json({ success: true, data: zone });
...
exports.updateZoneStatus = async (req, res) => {
  try {
    ...
    const updatedZone = await EventZone.updateStatus(req.params.id, req.body.status);
    res.status(200).json({ success: true, data: updatedZone });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
